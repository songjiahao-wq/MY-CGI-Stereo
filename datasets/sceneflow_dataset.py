...
class SceneFlowDatset(Dataset):
    def __init__(self, datapath, list_filename, training):
        ...
        self.cache = OrderedDict()  # 图像缓存
        self.cache_size = 500  # 最大缓存数

        self.disp_cache = OrderedDict()
    ...
    def load_image(self, filename):
        ...
        if filename in self.cache:
            img = self.cache.pop(filename)  # 移动到末尾（最新使用）
    ...
    def load_disp(self, filename):
        if filename in self.disp_cache:
            disp = self.disp_cache.pop(filename)
            self.disp_cache[filename] = disp
            return disp

        data, scale = pfm_imread(filename)
        if 'things' in filename:
            data = -data
        data = np.ascontiguousarray(data, dtype=np.float32)

        self.disp_cache[filename] = data
        if len(self.disp_cache) > self.cache_size:
            self.disp_cache.popitem(last=False)
        return data
