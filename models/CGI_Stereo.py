...
class CGI_Stereo(nn.Module):
    ...
    def forward(self, left, right):
        ...
        """feat_volume * corr_volume：
            将特征体积 feat_volume（通常来自左图的多尺度特征）与相关性体积 corr_volume（左右图特征相似度）逐元素相乘，增强匹配位置的响应。
            self.agg：
            3D卷积层，用于聚合局部上下文信息，生成初步的代价体 volume。
            self.hourglass_fusion：
            沙漏网络（Hourglass Network）或类似的3D编解码结构，进一步优化代价体 cost，结合左图的2D高层特征 features_left 补充空间细节。
        """
        volume = self.agg(feat_volume * corr_volume)
        cost = self.hourglass_fusion(volume, features_left)
        """self.spx_4 和 self.spx_2：
        空间金字塔模块（Spatial Pyramid, SPX），通过多尺度卷积（如空洞卷积）提取左图的高分辨率空间特征，用于后续的上采样。
        spx_pred：
        输出一个概率分布（通过 softmax），表示每个像素在不同视差方向上的偏移权重（用于亚像素级视差细化）。"""
        ...
        3. 视差回归
        disp_samples：
        生成视差候选值（从 0 到 maxdisp//4），形状调整为 [B, D, H, W]（与 cost 对齐）。
        
        regression_topk：
        基于代价体 cost 和候选视差，通过 Top-K 加权求和 回归最终的视差值（通常取 K=2，即用概率最高的两个候选视差插值）。"""
        ...
        """4. 上下文上采样
        context_upsample：
        利用 spx_pred 的空间偏移权重，对低分辨率视差图 pred 进行 亚像素级上采样，恢复高分辨率视差图 pred_up。
        具体实现可能是：
        
        将 pred 从 H/4 × W/4 上采样到 H × W。
        
        根据 spx_pred 的权重调整每个像素的视差值，消除锯齿效应。"""
...
"""关键点解析
1. 为什么需要 spx_pred？
直接上采样低分辨率视差图会导致边缘模糊，spx_pred 通过学习局部偏移权重，实现 细节保留的上采样。

2. regression_topk 的作用：
替代传统的 argmin 操作，通过加权插值得到更平滑的视差图。

3. maxdisp//4 的意义：
通常在计算代价体时会降低分辨率（如1/4尺度），因此候选视差范围也按比例缩小。
